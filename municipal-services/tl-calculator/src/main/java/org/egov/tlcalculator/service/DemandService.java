...
//			  BigDecimal.ZERO) .tenantId(tenantId).build()); });
			...
			case businessService_TL:
				if (license.getApplicationType() != null && license.getApplicationType().toString()
						.equals(TLCalculatorConstants.APPLICATION_TYPE_RENEWAL)) {
	...
	private List<Demand> updateDemand(RequestInfo requestInfo, List<Calculation> calculations, String businessService) {
		...
		for (Calculation calculation : calculations) {
			...
			List<Demand> searchResult = searchDemand(calculation.getTenantId(),
					Collections.singleton(calculation.getTradeLicense().getApplicationNumber()), requestInfo,
					businessService);

			if (CollectionUtils.isEmpty(searchResult))
				throw new CustomException("INVALID UPDATE", "No demand exists for applicationNumber: "
						+ calculation.getTradeLicense().getApplicationNumber());
