...
@Service
@Slf4j
public class ChangeBeneficialWorkflowIntegrator {
	...
	@Autowired
	public ChangeBeneficialWorkflowIntegrator(RestTemplate rest, TLConfiguration config) {
		this.rest = rest;
		this.config = config;
	}
	 ...
	 */
	public void callWorkFlow(List<Map<String,Object>> workFlowRequests,
			String businessServiceFromMDMS,RequestInfo requestInfo,
			List<Document> wfDocuments,List<String> assignees) {
		if (businessServiceFromMDMS == null)
			businessServiceFromMDMS = businessService_TL;
		JSONArray array = new JSONArray();
		
		for (Map<String,Object> workflowRequest : workFlowRequests) {
			if((businessServiceFromMDMS.equals(businessService_TL))||(! workflowRequest.get("action").toString().equalsIgnoreCase(TRIGGER_NOWORKFLOW))) {
				...
				obj.put(BUSINESSIDKEY, workflowRequest.get("cbApplicationNumber"));
				obj.put(TENANTIDKEY, workflowRequest.get("wfTenantId"));
				switch(businessServiceFromMDMS)
				{
				//TLR Changes
					case businessService_TL:
						obj.put(BUSINESSSERVICEKEY, workflowRequest.get("workflowCode"));
					...
					case businessService_BPA:
						String tradeType = workflowRequest.get("workFlowRequestType").toString(); 
				...
				obj.put(ACTIONKEY,  workflowRequest.get("action"));
				obj.put(COMMENTKEY,  workflowRequest.get("comment"));
				if (!CollectionUtils.isEmpty(assignees))
					obj.put(ASSIGNEEKEY, uuidmaps);
				obj.put(DOCUMENTSKEY, wfDocuments);
				array.add(obj);
			}
		}
		if(!array.isEmpty())
