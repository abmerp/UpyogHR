...
//	private TLRowMapper rowMapper;
	...
	String queryForgetTradeLicenseDetails="select eg_tl_tradelicense.validFrom,eg_tl_tradelicense.validTo,eg_tl_tradelicensedetail.licensefeecharges,eg_tl_tradelicensedetail.applicationnumber from eg_user inner join eg_tl_tradelicense on eg_user.uuid=eg_tl_tradelicense.createdBy "
			+ "inner join eg_tl_tradelicensedetail on eg_tl_tradelicensedetail.tradelicenseid = eg_tl_tradelicense.id "
...
//	}
   ...
   public ChangeBeneficial getBeneficialByLicenseNumber(String licenseNumber) {
		...
		try {
		List<Object> preparedStmtList = new ArrayList<>();
		List<ChangeBeneficial> changeBeneficial = jdbcTemplate.query(queryForGetChangeBeneficial.replaceAll(":licenseNumber", "'"+licenseNumber+"'"), preparedStmtList.toArray(),  (rs, rowNum) ->ChangeBeneficial.builder()
				.id(rs.getString("id").toString())
				.developerServiceCode(rs.getString("developerServiceCode").toString())
