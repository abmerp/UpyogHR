...
public class TLQueryBuilder {
	...
	private static final String QUERY = "SELECT tl.*,tld.*,tlunit.*,tlacc.*,tlowner.*,"
			...
			+ INNER_JOIN_STRING + "eg_tl_tradelicensedetail tld ON tld.tradelicenseid = tl.id" + LEFT_OUTER_JOIN_STRING
			+ "eg_tl_address tladdress ON tladdress.tradelicensedetailid = tld.id" + LEFT_OUTER_JOIN_STRING
			+ "eg_tl_owner tlowner ON tlowner.tradelicensedetailid = tld.id" + LEFT_OUTER_JOIN_STRING
	...
	public String getTLSearchQuery(TradeLicenseSearchCriteria criteria, List<Object> preparedStmtList,
			RequestInfo requestInfo, boolean isCount) {
		...
		if (criteria.getAccountId() != null) {
			...
			if (criteria.getTcpApplicationNumber() != null) {
				...
				addClauseIfRequired(preparedStmtList, builder);
				builder.append(" LOWER(tl.tcpapplicationnumber) IN (").append(createQuery(tcpApplicationNumber))
						.append(")");
...
//            }
	...
	private void addRenewalCriteria(StringBuilder builder, List<Object> preparedStmtList,
		 ...
		 * latest financial year among all the renewal application for that particular
		 * license number
		 */
		builder.append(
