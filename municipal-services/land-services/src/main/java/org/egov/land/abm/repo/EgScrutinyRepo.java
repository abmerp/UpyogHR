...
public interface EgScrutinyRepo extends JpaRepository<EgScrutiny, Long> {
	...
	@Query(value = "select s from EgScrutiny s where s.applicationId=?1 and s.isApproved not in ('Noting','Proforma')  order by created_on DESC")
	public List<EgScrutiny> findByApplicationSearch(String applicationNumber);

	@Query(value = "select s from EgScrutiny s where s.applicationId=?1 and s.isApproved not in ('Not In Order','Order With Conditions','In Order')  order by created_on ASC")
	public List<EgScrutiny> findByApplicationSearch5(String applicationNumber);

	@Query(value = "select s from EgScrutiny s where s.applicationId=?1 and s.role in(?2) and s.isApproved not in ('Noting','Proforma') order by created_on DESC")
...
//			String status);
	...
	@Query(value = "select s from EgScrutiny s where s.applicationId=?1 and s.fieldIdL=?2 and s.userid=?3 and s.serviceId=?4 and s.applicationStatus =?5")
	public EgScrutiny isExistsByApplicationIdAndFieldIdLAndUseridAndServiceId(String applicationId, String fieldId,
			Integer userID, Integer serviceId, String applicationStatus);
}
