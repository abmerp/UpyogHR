...
public class UserRepository {
	...
	public List<User> findAll(UserSearchCriteria userSearch) {
		...
		if (userSearch.getParentid() != null && userSearch.getParentid() > 0 && userSearch.getTenantId() != null) {
			...
			return findUserWithParentId(userSearch);
	 ...
	 * @return
	 */
	private List<User> findUserWithParentId(UserSearchCriteria userSearch) {
		final List<Object> preparedStatementValues = new ArrayList<>();

		String queryStr = userTypeQueryBuilder.getQueryUserParentSearch(userSearch, preparedStatementValues);
		log.debug(queryStr);
		List<User> users = new ArrayList<>();
		// usersIds = jdbcTemplate.queryForList(queryStr,
		// preparedStatementValues.toArray(), Long.class);
		users = jdbcTemplate.query(queryStr, preparedStatementValues.toArray(), userResultSetExtractor);
		enrichRoles(users);
		return users;
	}
